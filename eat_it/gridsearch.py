...
class GridSearchAvgCV(BaseSearchCV)):
    """Base class for hyper parameter search with cross-validation."""
    ...
    def _fit(self, X, y, parameter_iterable):
        ...
        cv_list = [check_cv(cv, X, y, classifier=is_classifier(estimator))
                    for cv in cv_list]

        if self.verbose > 0:
            if isinstance(parameter_iterable, Sized):
                n_candidates = len(parameter_iterable)
                print("Fitting {0} folds for each of {1} candidates, for "
                      " {2} CV repeats".format(len(cv), n_candidates,
                                         n_candidates * len(cv_list)))
        ...
        for cv in cv_list:
            ...
            for train, test in cv:
                ...
                y_pred[test_index] = clf.predict(X[test])
